...
{
  for (int I=(int)unrar_wcslen(Path)-1;I>=0;I--)
    if (IsPathDiv(Path[I]))
...
{
  size_t Length=unrar_wcslen(Path);
...
wchar* ConvertPath(const wchar *SrcPath,wchar *DestPath)
{
  ...
  while (*DestPtr!=0)
  {
    ...
    if (s[0]=='\\' && s[1]=='\\')
    {
      const wchar *Slash=unrar_wcschr(s+2,'\\');
      if (Slash!=NULL && (Slash=unrar_wcschr(Slash+1,'\\'))!=NULL)
        s=Slash+1;
    }
...
void SetExt(wchar *Name,const wchar *NewExt)
{
  ...
  if (NewExt==NULL)
  {
    ...
    {
      unrar_wcscat(Name,L".");
      unrar_wcscat(Name,NewExt);
    }
    else
      unrar_wcscpy(Dot+1,NewExt);
}
...
void AddEndSlash(wchar *Path,size_t MaxLength)
{
  ...
  if (Length>0 && Path[Length-1]!=CPATHDIVIDER && Length+1<MaxLength)
    unrar_wcscat(Path,PATHDIVIDERW);
}
...
void GetFilePath(const wchar *FullName,wchar *Path,size_t MaxLength)
{
  ...
  unrar_wcsncpy(Path,FullName,PathLength);
  Path[PathLength]=0;
}
...
wchar* GetVolNumPart(const wchar *ArcName)
{
  ...
  const wchar *ChPtr=ArcName+unrar_wcslen(ArcName)-1;
  ...
  while (NumPtr>ArcName && *NumPtr!='.')
  {
    if (IsDigit(*NumPtr))
    {
      ...
      // before it.
      wchar *Dot=unrar_wcschr(PointToName(ArcName),'.');
...
void NextVolumeName(wchar *ArcName,uint MaxLength,bool OldNumbering)
{
  ...
  if ((ChPtr=GetExt(ArcName))==NULL)
  {
    ...
    if ((ChPtr[1]==0 && unrar_wcslen(ArcName)<MaxLength-3) || wcsicomp(ChPtr+1,L"exe")==0 || wcsicomp(ChPtr+1,L"sfx")==0)
      unrar_wcscpy(ChPtr+1,L"rar");
  if (!OldNumbering)
  {
    ...
    while ((++(*ChPtr))=='9'+1)
    {
      *ChPtr='0';
      ...
      {
        for (wchar *EndPtr=ArcName+unrar_wcslen(ArcName);EndPtr!=ChPtr;EndPtr--)
          *(EndPtr+1)=*EndPtr;
        *(ChPtr+1)='1';
        break;
      }
...
{
#ifndef _UNIX
  if (Name[0] && Name[1] && unrar_wcschr(Name+2,':')!=NULL)
    return false;
...
void MakeNameUsable(wchar *Name,bool Extended)
{
  ...
  {
    if (unrar_wcschr(Extended ? L"?*<>|\"":L"?*",*s)!=NULL || (Extended && (uint)*s<32))
      *s='_';
#ifndef _UNIX
    if (s-Name>1 && *s==':')
      *s='_';
...
wchar* UnixSlashToDos(wchar *SrcName,wchar *DestName,size_t MaxLength)
{
  if (DestName!=NULL && DestName!=SrcName)
  {
    if (unrar_wcslen(SrcName)>=MaxLength)
    {
      *DestName=0;
      return DestName;
    }
    else
      unrar_wcscpy(DestName,SrcName);
  }
...
void GetPathRoot(const wchar *Path,wchar *Root,size_t MaxSize)
{
  *Root=0;
  if (IsDiskLetter(Path))
    ...
    if (Path[0]=='\\' && Path[1]=='\\')
    {
      const wchar *Slash=unrar_wcschr(Path+2,'\\');
      if (Slash!=NULL)
      {
        size_t Length;
        if ((Slash=unrar_wcschr(Slash+1,'\\'))!=NULL)
          Length=Slash-Path+1;
        else
          Length=unrar_wcslen(Path);
        if (Length>=MaxSize)
          Length=0;
        unrar_wcsncpy(Root,Path,Length);
        Root[Length]=0;
      }
    }
}
...
uint ParseVersionFileName(wchar *Name,bool Truncate)
{
  int Version=0;
...
wchar* VolNameToFirstName(const wchar *VolName,wchar *FirstName,bool NewNumbering)
{
  if (FirstName!=VolName)
    unrar_wcscpy(FirstName,VolName);
  ...
  if (!FileExist(FirstName))
  {
    ...
    while (Find.Next(&FD))
    {
      ...
      {
        unrar_wcscpy(FirstName,FD.Name);
        break;
      }
    }
  }
...
static void GenArcName(wchar *ArcName,wchar *GenerateMask,uint ArcNumber,bool &ArcNumPresent)
{
  ...
  for (uint I=0;Mask[I]!=0;I++)
  {
    ...
    if (CurChar=='N')
    {
      ...
      if (NCount<Digits)
      {
        wmemmove(Mask+I+Digits,Mask+I+NCount,unrar_wcslen(Mask+I+NCount)+1);
  ...
  *Ext=0;
  if (Dot==NULL)
    unrar_wcscpy(Ext,*PointToName(ArcName)==0 ? L".rar":L"");
  else
  {
  ...
  for (int I=0;Mask[I]!=0;I++)
  {
    ...
    if (QuoteMode)
      continue;
    const wchar *ChPtr=unrar_wcschr(MaskChars,toupperw(Mask[I]));
    if (ChPtr!=NULL)
  ...
  *DateText=0;
  ...
  if (Prefix)
  {
    ...
    unrar_wcscpy(ArcName,NewName);
  }
  else
    unrar_wcscat(ArcName,DateText);
  unrar_wcscat(ArcName,Ext);
}
...
wchar* GetWideName(const char *Name,const wchar *NameW,wchar *DestW,size_t DestSize)
{
  ...
  {
    if (DestW!=NameW)
      unrar_wcsncpy(DestW,NameW,DestSize);
  }
  else
    if (Name!=NULL)
...
bool GetWinLongPath(const wchar *Src,wchar *Dest,size_t MaxSize)
{
  ...
  size_t SrcLength=unrar_wcslen(Src);
  if (IsFullPath(Src)) // Paths in d:\path\name format.
  {
    if (IsDiskLetter(Src))
    {
      if (MaxSize<=PrefixLength+SrcLength)
        return false;
      unrar_wcsncpy(Dest,Prefix,PrefixLength);
      unrar_wcscpy(Dest+PrefixLength,Src);
      return true;
    }
    else
      if (Src[0]=='\\' && Src[1]=='\\')
      {
        if (MaxSize<=PrefixLength+SrcLength+2)
          return false;
        unrar_wcsncpy(Dest,Prefix,PrefixLength);
        unrar_wcscpy(Dest+PrefixLength,L"UNC");
        unrar_wcscpy(Dest+PrefixLength+3,Src+1);
        return true;
      }
  ...
  else
  {
    ...
    if (IsPathDiv(Src[0])) // Paths in \path\name format.
    {
      if (MaxSize<=PrefixLength+SrcLength+2)
        return false;
      unrar_wcsncpy(Dest,Prefix,PrefixLength);
      unrar_wcsncpy(Dest+PrefixLength,CurDir,2); // Copy drive letter 'd:'.
      unrar_wcscpy(Dest+PrefixLength+2,Src);
      return true;
    }
    else  // Paths in path\name format.
    {
      AddEndSlash(CurDir,ASIZE(CurDir));
      if (MaxSize<=PrefixLength+unrar_wcslen(CurDir)+SrcLength)
        return false;
      unrar_wcsncpy(Dest,Prefix,PrefixLength);
      unrar_wcscpy(Dest+PrefixLength,CurDir);
